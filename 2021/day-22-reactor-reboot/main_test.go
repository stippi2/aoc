...
func Test_parseInput(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: []Volume{
				{on: true, min: Position{-20, -36, -47}, max: Position{26, 17, 7}},
			},
		},
	}
	for _, test := range tests {
		assert.Equal(t, test.expected, parseInput(test.input))
	}
}
...
var largeExamplePartTwo = `on x=-5..47,y=-31..22,z=-19..33
...
func Test_largeExamplePartTwo(t *testing.T) {
	sequence := parseInput(largeExamplePartTwo)
	volumes := rebootSequence(sequence)
	cubes := countCubes(volumes)
	assert.Equal(t, 2758514936282235, cubes)
}
...
func Test_volume(t *testing.T) {
	...
	for _, test := range tests {
		assert.Equal(t, test.expectedVolume, test.v.width() * test.v.height() * test.v.depth())
	}
}
...
func Test_subtract(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			Volume{
				min: Position{10, 10, 10},
				max: Position{20, 20, 40},
			},
			[]Volume{
				{
					min: Position{0, 0, 0},
					max: Position{30, 30, 9},
				},
				{
					min: Position{0, 21, 10},
					max: Position{30, 30, 30},
				},
				{
					min: Position{0, 0, 10},
					max: Position{30, 9, 30},
				},
				{
					min: Position{0, 10, 10},
					max: Position{9, 20, 30},
				},
				{
					min: Position{21, 10, 10},
					max: Position{30, 20, 30},
				},
			},
		},
	}
